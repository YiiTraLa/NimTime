...
int main()
{
	...
	int computerTotal = 0;
	int computerTurns = 0; //i want to do a while and this is how to see who is the loser
	int playerTurns = 0;
	...
	string playerName;
	string computer;
	cout << "Welcome to the game of NIM, this is a game between the computer and the player where the players take turns reomving objects from a heap." << endl;
	...
	do {
		...
		{
			cout << "ERROR: That is not one of the numbers you can input, please choose either 1, 2, or 3.";
			cin >> playerNum;
		} 
		
		itemBag = itemBag - playerNum; // subtracting the player number inputed from the bag
		playerTotal = playerNum + playerTotal;
		playerTurns = playerTurns + playerNum; // below im checking to see if player took out the last object
		if (itemBag <= 0) {
	...
	/*if (computerTurns < playerTurns) // adds up who got the most turns which makes them lose
	{
		cout << "Computer has won!" << endl;
	}
	else (cout << "Player has won!");

	cout << computerTurns << " " << playerTurns << endl; */
	
	// cout << "Computer's Total: " << computerTotal << ", Player's Total:  " << playerTotal << endl;
